...
### BASE
追求最终一致性，因为系统的故障，允许短暂的不一致，但是在故障恢复后，数据逐渐会达到一致性。
基本可用、软状态、最终一致性，即BASE
...
|   引擎名称    |                  优点                  |                                      缺点                                      |                           用途                           |
| ------------ | -------------------------------------- | ----------------------------------------------------------------------------- | ------------------------------------------------------- |
| InnoDB       | 事务支持、行级锁表、外键约束、崩溃恢复       | 启动慢(mysql启动时，需要在内存中建立缓冲池，用于缓冲数据和索引)、不支持全文搜索、不记录行数 | 适合对事务完整性要求比较高，在并发条件下对数据一致性比较高的环境。 |
| MyISAM       |                                        | 不支持事务，行级锁和外键约束                                                       |                                                         |
| Memory       | 把数据存放内存中，主用于临时表中            |                                                                               |                                                         |
| MyISAM Merge | 把相同的MyISAM合并成一张虚表              |                                                                               | 主要用于日志和数据仓库                                      |
...
|   名称   |                                                 描述                                                  |                                      创建方式                                      |
| ------- | ----------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------- |
| 普通索引 | 最基本的索引，没有任何限制                                                                                | ALTER TABLE `table_name` ADD INDEX index_name (`column`);                        |
| 唯一索引 | 唯一索引的列值必须唯一，允许为null                                                                         | ALTER TABLE `table_name` ADD UNIQUE index_name (`column`);                       |
| 主键索引 | 唯一索引，并且一张表只有一个主键，不允许为null                                                               | ALTER TABLE `table_name` ADD PRIMARY KEY (`column`);                             |
| 联合索引 | 同时在多个字段上创建索引，查询效率更高                                                                      | ALTER TABLE `table_name` ADD INDEX index_name (`column1`, `column2`, `column3`); |
| 全文索引 | 主要用来匹配字符串文本中关键字。当需要字符串中是否包含关键字，会使用到like，如果是'%'开头，索引会失效，可以使用全文索引 | ALTER TABLE `table_name` ADD FULLTEXT (`column`);                                |
...
|            创建索引             |                                                   说明                                                   |   不创建索引   |                       说明                        |
| ------------------------------- | -------------------------------------------------------------------------------------------------------- | -------------- | ------------------------------------------------- |
| 频繁查询字段                     | 每张表的字段都有冷热之分，频繁使用的字段就可以加索引                                                        | 频繁更新字段   | 会破坏B+树的索引结构，频繁更新索引文件，SQL性能变差 |
| 有序的字段                      | 插入数据库，仍能保持B+树的索引结构，不需要频繁更新索引文件，性能更好                                         | 无序的字段     | 会破坏B+树的索引结构，频繁更新索引文件，性能较差    |
| 区分度高的字段                   | 可以手动计算，如```select count(distinct age) / count(*) from table ```                                   | 区分度低的字段 |                                                   |
| where或on后面的字段              | 由执行顺序决定，```from > on > join > where > group by > having > select > distinct > order by > limit``` |                |                                                   |
|                                 |                                                                                                          | 过长的字段     | 会占用很多的空间                                   |
| order by 或group（建议，非必需） | 建议创建索引，避免使用文件排序                                                                             |                |                                                   |

索引区分度：
```
索引列中不重复的值的数量与表中总记录数的比例。
```
计算方式：
```sql
...
|               建议                |                                                         说明                                                         |
| -------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| 优先使用联合索引                  | 多个字段可以更精准匹配到所需数据                                                                                       |
| 使用联合索引，区分度高的字段放前面 | 减少查询次数，更快匹配所需的数据                                                                                       |
| 过长字符串可以使用前缀索引         | 当一个字符串过长，加索引消耗更多的存储空间，截取前几位创建索引，如创建语句，```alter table tables add index culumn(5)``` |
| 值唯一的，使用唯一索引             | 使用唯一索引，可以避免程序产生bug产生重复数                                                                            |
| order by或group by建议创建索引    | 避免文件排序，使用索引提高性能                                                                                         |
| 索引个数不建议太多，适度即可       | 过多的索引会占用更多的存储空间，影响SQL性能；更新SQL，需要更新大量索引文件                                               |

前缀索引：
某一个字段的值太长，但又很频繁的查询，创建索引有需要消耗更多的存储空间，只截取前几个字符创建索引。
缺点：无法使用前缀进行order by 或group by，也无法进行覆盖扫描
...
| 数据类型隐式转换           |                                                                                                                         |
| 模糊查询，like以%开头      |                                                                                                                         |
| or前后没有同时使用索引      | 前面字段使用索引，后面字段没有使用索引                                                                                        |
| 联合索引，没有使用第一列索引 | 必须遵循最左匹配原则，首先使用第一列字段，然后使用第二列字段; 如果创建的联合索引是(name,age),那么如果查询条件只有age,会使用索引进行全表扫描 |
| 在索引字段上使用计算        |                                                                                                                         |
| 在索引字段上使用函数        |                                                                                                                         |
| 优化器选错索引             |                                                                                                                         |
...
```sql
-- age 加了索引，name没加索引
select id, name, age from user where age = 3 and name = 'zj'; 
select id, name, age from user where age = 3 or name = 'zj'; 
```
分析该sql会不会走索引
如果是and连接符，会走索引
如果是or连接符，不会走索引，会全表扫描

##### MySQL查询性能优化七种武器
###### 索引下堆
* 作用
减少回表查询的次数
减少存储引擎和mysql server层的数据传输量，提高执行性能
* 何时使用到
执行计划中Extra中展示了Using index condition
* 查看是否开启、开启或关闭：
```sql
...
```
如果展示中有index_condition_pushdown = on表示已开启状态
* 原理

|                                                                 没有索引下堆                                                                 |                                                                               有索引下堆                                                                               |
| ------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1. 存储引擎根据where条件的索引字段，找到符合条件的主键id 2. 然后二次回表查询，使用主键id去主键索引上找到该整行记录 3. 把数据返回到mysql server层 4. 返回客户端 | 1. 存储引擎根据where条件的索引字段，找到符合条件的记录，在用其他条件查出符合条件的主键id 2. 然后二次回表查询，使用主键id去主键索引上找到该整行记录 3. 把数据返回到mysql server层 4. 返回客户端 |

* 场景

|                                                      |                                说明                                 |
| ---------------------------------------------------- | ------------------------------------------------------------------- |
| 适用于InnoDB和MyIsAm引擎的查询                          |                                                                     |
| 适用于执行计划是range、ref、rq_ref、rq_ref_null的范围查询 |                                                                     |
| 子查询不能使用索引下堆                                   |                                                                     |
| 存储过程不能使用索引下堆                                 |                                                                     |
| 对于InnoDB表，仅适用于非聚簇索引。不适合用于聚簇索引。       | 对于InnoDB聚簇索引，完整的记录已读取到InnoDB缓冲区中，使用索引下堆不会减少I/O |
...
|      列名      |                                                 说明                                                  |
| ------------- | ---------------------------------------------------------------------------------------------------- |
| id            | 表示查询语句的序号，自动分配，顺序递增，值越大，执行优先级越高。  id相同时，优先级由上而下                         |
| select_type   | 表示查询类型，常见的有SIMPLE简单查询、PRIMARY主查询、SUBQUERY子查询、UNION联合查询、UNION RESULT联合临时表结果等 |
| table         | 表示SQL语句查询的表名、表别名、临时表名                                                                    |
| partitions    | 表示SQL查询匹配到的分区，没有分区的话显示NULL                                                              |
| type          | 表示表连接类型或者数据访问类型，就是表之间通过什么方式建立连接的，或者通过什么方式访问到数据的                       |
| possible_keys | 表示可能用到的索引列，实际查询并不一定能用到                                                                |
| key           | 表示实际查询用到索引列                                                                                   |
| key_len       | 表示索引所占的字节数                                                                                    |
| ref           | 表示where语句或者表连接中与索引比较的参数，常见的有const（常量）、func（函数）、字段名。 如果没用到索引，则显示为NUL |
| rows          | 表示执行SQL语句所扫描的行数                                                                              |
| filtered      | 表示按条件过滤的表行的百分比。  用来估算与其他表连接时扫描的行数，row x filtered = 252004 x 10% = 25万行        |
| Extra         | 表示一些额外的扩展信息，不适合在其他列展示，却又十分重要                                                      |
...
|      值      |                               说明                               |
| ----------- | ---------------------------------------------------------------- |
| system      | 当表中只有一行记录，也就是系统表，是 const 类型的特列                   |
| const       | 表示使用主键或者唯一性索引进行等值查询，最多返回一条记录。性能较好，推荐使用 |
| eq_ref      | 表示表连接使用到了主键或者唯一性索引                                   |
| ref         | 表示使用非唯一性索引进行等值查询                                      |
| ref_or_null | 表示使用非唯一性索引进行等值查询，并且包含了null值的行                   |
...
|   分类   |               具体锁说明                |
| -------- | -------------------------------------- |
| 锁的粒度 | 表锁、页面锁、行锁、记录锁、间隙锁、临键锁 |
| 锁的属性 | 共享锁、排他锁                          |
| 锁的机制 | 乐观锁、悲观锁                          |

|      锁名称       |    支持引擎    |                        优点                         |                       缺点                       |                                                实现                                                 |         例子          |                                                         描述                                                          |             作用              |
| ----------------- | -------------- | --------------------------------------------------- | ----------------------------------------------- | --------------------------------------------------------------------------------------------------- | --------------------- | --------------------------------------------------------------------------------------------------------------------- | ----------------------------- |
| 表锁              | InnoDB、MyISAM | 开销小，加锁快，不会出现死锁                          | 锁定粒度大，发生锁冲突概率高，并发度低            |                                                                                                     |                       |                                                                                                                       |                               |
| 页面锁            | BDB            | 开销和加锁速度介于表锁和行锁之间                      | 会出现死锁，锁定粒度介于表锁和行锁之间，并发度一般 |                                                                                                     |                       |                                                                                                                       |                               |
| 行锁              | InnoDB         | 开销大，加锁慢；锁住力度小，发生所冲突概率较低，并发高 | 会发生死锁                                       |                                                                                                     | 记录锁、间隙锁、临键锁 |                                                                                                                       |                               |
| 记录锁            |                |                                                     |                                                 | update user set age = age + 1 where id = 1                                                          |                       | 对某条记录加锁，他封锁的是该行的索引记录，必须是主键列或唯一索引，否则加锁会变成临键锁                                      |                               |
| 间隙锁            |                |                                                     |                                                 | update user set age = age + 1 where id > 1 and id < 5                                               |                       | 对某个范围加锁，但不包含范围的临界数据                                                                                   |                               |
| 临键锁            |                |                                                     |                                                 | update user set age = age + 1 where id >= 1 and id <= 5                                             |                       | 由记录锁和间隙锁组成，包含临界条件                                                                                      | 解决幻读问题                  |
| 共享锁(读锁、S锁) |                |                                                     |                                                 | 在select语句后面加```lock in share mode  ```                                                         |                       |                                                                                                                       | 防止其他事务修改当前数据       |
| 排他锁(写锁、X锁) |                |                                                     |                                                 | 在select语句后面加```for update  ```                                                                 |                       |                                                                                                                       | 防止其他事务读取或更新当前数据 |
| 乐观锁            |                |                                                     |                                                 | select id,version from user where id = 1;update user set age = age + 1 where id = 1 and version = 1 |                       | 认为每次操作都不会修改数据，然后每一次读取数据都不加锁，在更新数据的时候就通过version来判断是否修改数据,适用于读多写少的情况 |                               |
| 悲观锁            |                |                                                     |                                                 | select * from user where id = 1 lock in share mode; select * from user where id = 1 for update      |                       | 认为每次操作都会修改数据，然后每一次读取数据都需要加锁，适用于写多读少的情况                                               |                               |
...
-- 通过年龄更新名称
update user set name  = '测试' where age = 2;
-- 通过id更新名称
update user set name  = '测试' where id = 2;
```
假设数据表每有如下数据：
...
|   更新方式   |                                                              非主键或唯一性索引                                                               |                                             主键或唯一性索引                                              |
| ------------ | -------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------- |
| 年龄更新数据 | 如果age=2记录不存在表中，就只会加一个间隔锁，如(1,4)；如果age=2记录存在表中，会加一个临键锁，同时也会向右遍历到不满足条件的范围，加锁,如(1,2],(2,3) |                                                                                                          |
| id更新数据   |                                                                                                                                              | 如果id=2记录不存在表中，会在该范围加间隔锁，同时会向右遍历到不满足条件的范围；如果id=2记录存在表中，只加记录锁 |

总结：
1.  mysql中的锁是加在索引上面的
2. 如果非主键或唯一索引，无论记录是否存在表中，除了会在该记录范围内加锁，还会向右遍历到不满足条件的范围加锁
3. 如果是主键或唯一索引，如果记录存在表中，只加记录锁；如果记录不存在表中，就会在该记录范围内加锁，还会向右遍历到不满足条件的范围加锁
...
### 领域、子域、限界上下文
#### 领域
领域是一个组织所做的事情以及其中所包含的一切。由于领域一词意义表达很广泛，可能表示整个业务系统，也可能表示其中的某个的核心域或支撑子域。
#### 子域
包含：核心域、支撑子域、通用子域
核心域：是业务领域的一部分
支撑子域：专注于业务的某个方面
...
不建议在聚合中注入资源库及领域服务，而在其他多数情况下，依赖注入都是合适的。比如，可以向应用服务注入资源库及领域服务。
